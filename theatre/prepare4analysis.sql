...
FROM trips,central_park_weather_observations w
WHERE w.date = date(pickup_datetime) AND
ST_DWithin(pickup, ST_SetSRID(ST_Point(-73.9849902287, 40.76123555), 4326), 100) AND EXTRACT(hour FROM pickup_datetime) = 22 AND EXTRACT(minute FROM pickup_datetime)
<30 AND tip_amount>1 AND passenger_count>=2 GROUP BY 1,2,3,4,5,6;
